...
const SupportButton = ({ onClick }: { onClick: () => void }) => {
  return (
    <button
      onClick={onClick}
      className="fixed bottom-8 right-8 w-16 h-16 bg-[#D4AF37] rounded-full flex items-center justify-center text-black shadow-xl shadow-[#D4AF37]/30 hover:scale-110 transition-all active:scale-95 z-[60] group border-4 border-[#0B0B0F]"
...
const InfiniteMarquee = () => {
  return (
    <div className="w-full bg-[#12121A] border-y border-white/5 py-4 overflow-hidden select-none">
      <div className="animate-marquee whitespace-nowrap">
        {[...CONFIG.content.marquee, ...CONFIG.content.marquee].map((item, i) => (
          <div key={i} className="flex items-center mx-6 md:mx-10">
            <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] text-[#B7B7C2] flex items-center gap-4">
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 px-4 md:px-6 py-4 flex justify-center transition-all duration-500">
      ...
      >
        ...
        <div className="hidden lg:flex items-center gap-8">
          {CONFIG.content.navbar.map((link) => (
            <a key={link} href={`#${link.toLowerCase().replace(/é/g, 'e')}`} className="text-[10px] font-bold text-[#B7B7C2] hover:text-[#D4AF37] transition-all uppercase tracking-[0.2em] relative group">
              {link}
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setIsOpen(!isOpen)} className="lg:hidden text-white ml-2 p-2 rounded-lg bg-white/5 border border-white/10">
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {CONFIG.content.navbar.map((link) => (
              <a key={link} href={`#${link.toLowerCase().replace(/é/g, 'e')}`} onClick={() => setIsOpen(false)} className="text-3xl font-black text-white hover:text-[#D4AF37] tracking-tighter transition-colors">{link}</a>
            ))}
            <div className="h-[1px] bg-white/10 w-full my-2" />
            <a href={`tel:${CONFIG.contact.phone}`} className="flex items-center gap-3 text-white font-bold text-lg"><Phone size={20} className="text-[#D4AF37]" /> {CONFIG.contact.phone}</a>
          </motion.div>
        )}
      </AnimatePresence>
    </nav>
...
const SectionHeader = ({ title, subtitle, highlight }: { title: string; subtitle?: string; highlight?: string }) => (
  <div className="text-center mb-16 md:mb-24 max-w-4xl mx-auto px-4">
    ...
    <motion.h2 
      ...
      className="text-4xl md:text-6xl lg:text-[80px] font-black tracking-tighter mb-6 md:mb-8 leading-[0.9] text-white"
    >
      {title}
    </motion.h2>
    {subtitle && (
      <motion.p 
        ...
        className="text-[#B7B7C2] text-base md:text-xl font-medium leading-relaxed max-w-2xl mx-auto opacity-70"
      >
        {subtitle}
      </motion.p>
    )}
  ...
  return (
    <div className="min-h-screen bg-[#0B0B0F] selection:bg-[#D4AF37] selection:text-black overflow-x-hidden">
      ...
      <section id="accueil" className={`relative min-h-screen flex items-center pt-32 pb-16 px-4 md:px-6 overflow-hidden`}>
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[100%] h-[500px] md:h-[800px] bg-[#D4AF37]/5 rounded-full blur-[150px] md:blur-[200px] -z-10" />
        ...
        <div className="max-w-7xl mx-auto flex flex-col items-center text-center">
          ...
          <motion.p 
            ...
            className="text-[#B7B7C2] text-lg md:text-2xl font-medium max-w-2xl mb-12 md:mb-20 leading-relaxed opacity-80"
          >
          ...
          >
            ...
            <a href="#services" className="w-full sm:w-auto px-12 md:px-16 py-6 md:py-7 glass-dark border border-white/10 text-white rounded-2xl font-black text-sm md:text-xl hover:border-[#D4AF37]/50 transition-all text-center">
      ...
      {/* 2. SERVICES SECTION */}
      <section id="services" className={`${CONFIG.theme.spacing.sectionPadding} px-4 md:px-6 bg-[#07070D]`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
            {CONFIG.services.map((service, i) => (
              <motion.div 
                ...
                className="group relative h-[500px] md:h-[600px] rounded-[3.5rem] overflow-hidden border border-white/5 bg-[#12121A] cursor-pointer shadow-2xl"
              >
                ...
                <div className="relative z-10 h-full p-12 flex flex-col justify-end items-start text-left">
                   ...
                   <p className="text-base md:text-lg text-[#B7B7C2] font-medium leading-relaxed opacity-70 group-hover:opacity-100 transition-opacity">
                      {service.desc}
                   </p>
                </div>
      ...
      <section className={`${CONFIG.theme.spacing.sectionPadding} px-4 md:px-6 relative overflow-hidden flex justify-center`}>
         <div className="absolute inset-0 z-0">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-b from-[#0B0B0F] via-[#0B0B0F]/90 to-[#0B0B0F]" />
         </div>
         ...
         <div className="max-w-5xl mx-auto relative z-10 text-center">
            ...
            <motion.h2 {...fadeInUp} transition={{delay:0.1}} className="text-6xl md:text-8xl lg:text-9xl font-black text-white mb-12 tracking-tighter leading-[0.85]">
            ...
            <motion.p {...fadeInUp} transition={{delay:0.2}} className="text-xl md:text-3xl text-[#B7B7C2] font-medium mb-16 max-w-3xl mx-auto opacity-80 leading-relaxed">
            ...
            <motion.div {...fadeInUp} transition={{delay:0.3}} className="flex flex-col sm:flex-row gap-6 justify-center items-center">
               ...
               <a href={`tel:${CONFIG.contact.phone}`} className="w-full sm:w-auto px-14 py-7 border-2 border-white/20 text-white rounded-[2rem] font-black text-xl flex items-center justify-center gap-4 hover:border-[#D4AF37] hover:bg-white/5 transition-all">
      ...
      <section id="tarifs" className={`${CONFIG.theme.spacing.sectionPadding} px-4 md:px-6`}>
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-14 max-w-6xl mx-auto">
            <motion.div {...fadeInUp} className="p-10 md:p-16 rounded-[4rem] glass-dark border border-white/5 relative group overflow-hidden">
              <h3 className="text-3xl md:text-5xl font-black mb-10 md:mb-16 text-white tracking-tighter">Trajets Normandie</h3>
              <div className="space-y-6 md:space-y-10">
                {[
                  ...
                  <div key={i} className="flex justify-between items-center py-5 md:py-7 border-b border-white/5">
                    <span className="text-[#B7B7C2] text-lg md:text-xl font-bold">{item.label}</span>
            ...
            <motion.div {...fadeInUp} transition={{ delay: 0.1 }} className="p-10 md:p-16 rounded-[4rem] bg-white/[0.03] border border-white/10 relative group overflow-hidden shadow-2xl">
              ...
              <div className="space-y-6 md:space-y-10">
                {CONFIG.pricing.forfaits.slice(0, 4).map((f) => (
                  <div key={f.match} className="flex justify-between items-center py-5 md:py-7 border-b border-white/10">
                    <span className="text-[#B7B7C2] text-lg md:text-xl font-bold">Vers {f.name}</span>
                    <span className="text-2xl md:text-4xl font-black text-white">{f.price} €</span>
                  </div>
                ))}
              </div>
            </motion.div>
      ...
      {/* 5. RÉSERVER */}
      <section id="reserver" className={`${CONFIG.theme.spacing.sectionPadding} px-4 md:px-6 bg-[#07070D]`}>
        <div className="max-w-7xl mx-auto">
          <SectionHeader 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 md:gap-16 items-start">
            <motion.div {...fadeInUp} className="lg:col-span-7 glass-dark p-8 md:p-16 rounded-[4rem] border border-white/10 shadow-3xl">
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 md:gap-10 mb-12 md:mb-16">
                  <div className="col-span-1 sm:col-span-2">
                    <label className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] text-[#B7B7C2] mb-5 block">Type de Destination</label>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                      {['local', 'airport', 'long', 'pro'].map(t => (
                        <button key={t} onClick={() => setFormData({...formData, tripType: t})} className={`py-4 md:py-5 rounded-2xl font-black text-[10px] md:text-[11px] uppercase tracking-widest border-2 transition-all ${formData.tripType === t ? 'bg-[#D4AF37] text-black border-[#D4AF37] shadow-xl shadow-[#D4AF37]/20' : 'bg-[#0B0B0F] text-[#B7B7C2] border-[#242433] hover:border-white/20'}`}>
                  ...
                  <div className="space-y-3">
                    <label className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] text-[#B7B7C2] block">Lieu de départ</label>
                    <div className="relative">
                      <MapPin size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-[#D4AF37]" />
                      <input type="text" placeholder="Adresse complète" className="w-full bg-[#0B0B0F] border-2 border-[#242433] rounded-2xl pl-12 pr-5 py-5 text-white outline-none focus:border-[#D4AF37] text-sm md:text-base font-bold transition-all" onChange={e => setFormData({...formData, pickup: e.target.value})} />
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] text-[#B7B7C2] block">Lieu d'arrivée</label>
                    <div className="relative">
                      <Compass size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-[#D4AF37]" />
                      <input type="text" placeholder="Aéroport, Gare, Ville..." className="w-full bg-[#0B0B0F] border-2 border-[#242433] rounded-2xl pl-12 pr-5 py-5 text-white outline-none focus:border-[#D4AF37] text-sm md:text-base font-bold transition-all" onChange={e => setFormData({...formData, dropoff: e.target.value})} />
                    </div>
                  </div>

                  <div className="space-y-3">
                    <label className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] text-[#B7B7C2] block">Date du trajet</label>
                    <input type="date" className="w-full bg-[#0B0B0F] border-2 border-[#242433] rounded-2xl px-6 py-5 text-white outline-none font-bold text-sm md:text-base focus:border-[#D4AF37]" onChange={e => setFormData({...formData, date: e.target.value})} />
                  </div>

                  <div className="space-y-3">
                    <label className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] text-[#B7B7C2] block">Heure de prise en charge</label>
                    <input type="time" className="w-full bg-[#0B0B0F] border-2 border-[#242433] rounded-2xl px-6 py-5 text-white outline-none font-bold text-sm md:text-base focus:border-[#D4AF37]" onChange={e => setFormData({...formData, time: e.target.value})} />
                  </div>
               </div>

               <button onClick={calculateEstimate} className="w-full py-6 md:py-8 bg-white text-black rounded-3xl font-black text-lg md:text-2xl hover:bg-[#D4AF37] transition-all flex items-center justify-center gap-4 group active:scale-[0.98]">
            ...
            <div className="lg:col-span-5 flex flex-col gap-8 h-full">
              <AnimatePresence mode="wait">
                {estimate ? (
                  <motion.div key="est" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-[#12121A] p-10 md:p-16 rounded-[4rem] border-2 border-[#D4AF37]/40 flex-1 relative overflow-hidden shadow-3xl">
                     <div className="text-6xl md:text-8xl font-black text-white mb-6 tracking-tighter">{estimate.price}<span className="text-2xl md:text-3xl ml-2 text-[#D4AF37]">€</span></div>
                     <p className="text-base md:text-lg font-medium text-[#B7B7C2] opacity-80 mb-12 md:mb-16 leading-relaxed italic">
                        "{estimate.rule}"
                     </p>
                     ...
                     <div className="space-y-5">
                        ...
                        <button onClick={() => window.open(`https://wa.me/${CONFIG.contact.whatsapp}`, '_blank')} className="w-full py-6 border-2 border-[#242433] text-white rounded-2xl font-black text-lg md:text-xl flex items-center justify-center gap-3 hover:bg-white/5 transition-all">
                          <MessageCircle size={22} className="text-[#25D366]" /> PAYER EN ESPÈCES
                        </button>
                     </div>
                  </motion.div>
                ) : (
                  <div className="h-full bg-white/[0.02] border-2 border-white/5 border-dashed rounded-[4rem] flex flex-col items-center justify-center text-center p-12 min-h-[400px]">
                     <Zap size={40} className="text-white opacity-20 mb-10" />
                     <p className="text-lg md:text-xl text-[#B7B7C2] font-medium leading-relaxed opacity-40 max-w-xs">Complétez les informations pour débloquer votre tarif sur-mesure.</p>
                  </div>
                )}
              </AnimatePresence>
            </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12 text-center">
             {[
               ...
               <motion.div key={i} {...fadeInUp} transition={{ delay: i * CONFIG.theme.animations.stagger }} className="p-12 rounded-[3.5rem] glass-dark border border-white/5 flex flex-col items-center">
                  ...
                  <p className="text-[#B7B7C2] font-medium leading-relaxed opacity-60">{item.text}</p>
               </motion.div>
             ))}
          </div>
        </div>
      ...
      {/* 7. CONTACT SECTION */}
      <section id="contact" className={`${CONFIG.theme.spacing.sectionPadding} px-4 md:px-6 bg-[#07070D]`}>
        <div className="max-w-7xl mx-auto">
          <SectionHeader 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
             {[
               ...
               <motion.a 
                ...
                {...fadeInUp} 
                transition={{ delay: i * CONFIG.theme.animations.stagger }} 
                className="p-10 rounded-[3rem] glass-dark border border-white/5 flex flex-col items-center group hover:border-[#D4AF37]/40 transition-all shadow-xl"
               >
                  ...
                  <h3 className="text-xl font-black text-white mb-3 tracking-tight">{contact.title}</h3>
                  <p className="text-[#B7B7C2] font-medium opacity-60 text-sm">{contact.value}</p>
               </motion.a>
             ))}
          </div>
        </div>
      ...
      {/* 8. FOOTER */}
      <footer className={`${CONFIG.theme.spacing.sectionPadding} px-4 md:px-6 bg-[#0B0B0F] border-t border-white/5 relative`}>
        <div className="max-w-7xl mx-auto flex flex-col items-center text-center gap-16 md:gap-24">
          <div className="max-w-2xl">
             <span className="font-black text-4xl md:text-6xl tracking-tighter text-white mb-8 block">CHAUFFEUR <span className="text-[#D4AF37]">PRIVÉ</span></span>
             <p className="text-[#B7B7C2] text-lg md:text-xl font-medium leading-relaxed opacity-50 mb-10">
             ...
             <div className="flex justify-center gap-6 mb-12">
                {[
                  ...
                  <a key={i} href={social.link} target="_blank" rel="noreferrer" className="w-12 h-12 rounded-full glass-dark border border-white/10 flex items-center justify-center text-[#B7B7C2] hover:text-[#D4AF37] hover:border-[#D4AF37]/40 transition-all">
                    {social.icon}
                  </a>
                ))}
             </div>
             <div className="flex justify-center gap-8">
                <a href={`tel:${CONFIG.contact.phone}`} className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center text-[#D4AF37] border border-white/10 hover:bg-[#D4AF37] hover:text-black transition-all shadow-xl"><Phone size={24}/></a>
                <a href={`https://wa.me/${CONFIG.contact.whatsapp}`} className="w-16 h-16 rounded-2xl bg-white/5 flex items-center justify-center text-[#D4AF37] border border-white/10 hover:bg-[#D4AF37] hover:text-black transition-all shadow-xl"><MessageCircle size={24}/></a>
             </div>
          </div>
          
          <div className="flex flex-wrap justify-center gap-10 md:gap-20 text-[10px] md:text-[12px] font-black uppercase tracking-[0.4em] text-[#B7B7C2]">
             {CONFIG.content.navbar.map(l => (
               <a key={l} href={`#${l.toLowerCase().replace(/é/g, 'e')}`} className="hover:text-white transition-colors">{l}</a>
             ))}
          </div>
          
          <div className="w-full pt-16 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 opacity-40 text-[10px] font-black uppercase tracking-[0.4em]">
